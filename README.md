...
```sh
sudo apt install ./netease-cloud-music-webkit_1.0.3_all.deb
```
...
v1.0.3 修复扫码登录后停留在旧版登录界面的问题：进入官方登录页后，应用监听登录 Cookie 的变化，并在完成登录后返回播放器。如果网页已经自行返回播放器，不会再次刷新；播放期间的普通 Cookie 更新也不会触发跳转。登录在独立弹窗中完成时，仅关闭对应登录弹窗，保留主窗口及其他弹窗。
...
已移除顶部工具栏与标题栏，网页直接从窗口顶边显示。最小化、最大化／还原、关闭三个原生按钮集成在内容区右上角，网易云顶栏右侧留出空间，避免遮挡账号和皮肤等入口。F10 不再恢复工具栏。Alt+Home 返回播放器首页，Alt+F4 关闭窗口。弹窗同样使用内部窗口按钮；视频全屏时暂时隐藏按钮，退出全屏后恢复。

## 窗口显示

右上角窗口按钮采用透明底和灰蓝细线图标，与网页顶栏保持接近的视觉风格；只有悬停时显示轻微反馈，关闭按钮悬停时变为网易云红色。

v1.0.3 为无边框窗口补充了原生拖动和缩放区域，无需按住辅助键：

- 在窗口顶部、导航内容上方的空白细条按住鼠标左键拖动，即可移动窗口；双击该区域可最大化或还原。
- 拖动窗口的任意边缘或四角即可调整大小，最小尺寸为 640 × 480。最大化时先还原，再调整尺寸。
- 视频全屏时隐藏拖动和缩放区域，避免截获播放器操作。Ubuntu 默认也可按住 Super（Windows 徽标键）拖动窗口。

拖动和缩放直接交给系统窗口管理器处理；窗口尺寸和状态变化不会触发应用注入样式重建或网页布局查询。
...
生成 `netease-cloud-music-webkit_1.0.3_all.deb` 和 `netease-cloud-music-1.0.3-source.tar.gz`。
